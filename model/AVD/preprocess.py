...
from model.util import argparse_helper, cpu_parallel_process
...
def extract_waves(arguments):
	...
	for video in tqdm(videos, desc='Extracting waves'):
		...
		command = [
				...
				wave_path,						# Output file path and name
		...
		wavfile.write(denoised_path, rate, reduced_noise)


def crop_align_faces(arguments):
	videos, args = arguments
	...
	for video in tqdm(videos, desc='Cropping and aligning faces'):
		...
		frames_path = f'{args.data_path}/{args.track_folder}/aligned_tracklets/{uid}'
		os.makedirs(frames_path, exist_ok=True)

		track = read_track(f'{args.data_path}/{args.track_folder}/predictions/{uid}.csv')
		if track is None:
			continue

		cam = cv2.VideoCapture(video)
		for _, track_row in tqdm(track.iterrows(), desc=f'Processing track for {uid}', total=len(track), leave=False):
...
def crop_and_align_frame(track_row, frame, aligner):
	...
	# aligned_face_crop = RetinaFace.extract_faces(img_path=face_crop, align = True, )
	# return face_crop if len(aligned_face_crop) == 0 else aligned_face_crop[0]
...
def initialize_arguments(**kwargs):
	...
	parser.add_argument('--track_folder', type=str, default="asd/ground_truth", help='Name of folder containing tracks to use in processing')
	parser.add_argument('--n_threads',  	type=int, default=1, help='Number of threads for preprocessing')
...
def main(**kwargs):
	...
	cpu_parallel_process(extract_waves, videos, args.n_threads, args)
	cpu_parallel_process(crop_align_faces, videos, args.n_threads, args)
